...
A Second Look at Transferable Targeted Attacks](https://proceedings.neurips.cc/paper/2021/file/30d454f09b771b9f65e3eaf6e00fa7bd-Paper.pdf), [Code](https://github.com/ZhengyuZhao/Targeted-Tansfer): **No need for individual test**. `loss function` - uses logits loss instead of traditional cross entropy loss and uses larger iterations. Seems to help more with targeted attack, but not untargeted attacks. There is no need to individually test this method, instead, many follow-up works consider this as a way to boost the performance of their method (for example, `Boosting the Transferability of Adversarial Attacks with Reverse Adversarial Perturbation`).
    * [A Little Robustness Goes a Long Way: Leveraging Robust Features for Targeted Transfer Attacks](https://proceedings.neurips.cc/paper/2021/file/50f3f8c42b998a48057e9d33f4144b8b-Paper.pdf), [No Code]: **Contacted** `source model type` - published at NeurIPS 2021, no code is available, but should be easy to implement. Main message from the paper is, slightly robust source models show better transferability to unknown target models, once the local attacks are using with the attack listed above.
    * [Boosting the Transferability of Adversarial Attacks with Reverse Adversarial Perturbation](https://openreview.net/forum?id=i7FNvHnPvPc) [No Code]: **Contacted** `loss function` - Unpublished and no code available. Main messsage is that, instead of minimizing loss of single adversarial example, we should seek adversarial examples locating at the low-value and flat region of the loss landscape. Should still be feasible to implement on our own, as the main method is using min-max optimization iteratively.
    ...
    * [Adaptive Image Transformations for Transfer-based Adversarial Attack](https://arxiv.org/abs/2111.13844), [No Code]: **Contacted** `Input` - Adaptive input transformation for better transferability that requires training a encoder-decoder network such that it can output the optimal combination of existing input transformation techniques. This cannot be achieved in the `No-Auxiliary` information setting, though. If the author did not respond, an alternative (but may not be worthwhile) method is the [AutoMA](https://ieeexplore.ieee.org/abstract/document/9599534), [Code](https://github.com/HaojieYuan/autoAdv). Another alternative is [Improving the Transferability of Adversarial Samples with Adversarial Transformations](https://openaccess.thecvf.com/content/CVPR2021/papers/Wu_Improving_the_Transferability_of_Adversarial_Samples_With_Adversarial_Transformations_CVPR_2021_paper.pdf), but no code and the authors is not responding (from adaptive image transformation paper), I have contacted the authors, but not expecting a response. Their evaluation also seems to be problametic.
    * [Improving Adversarial Transferability with Spatial Momentum](https://arxiv.org/abs/2203.13479), [No Code]: **Contacted** `Gradient Computation` - spatial momentum method for boosted transferability.
...
2. Requires additional model (binary classifier, generator, some pretrained models):
    ...
    * [On Generating Transferable Targeted Perturbations](https://arxiv.org/abs/2103.14641), [Code](https://github.com/Muzammal-Naseer/TTP): (**Excluded**) Published at ICCV 2021, requires training a generator for target class feature distribution. This method is outperformed by the first attack of this list in the targeted setting and is also designed only for targeted setting.
...
2. Requires access to training data/distribution
    * [Semantics aware adversarial malware examples generation for black-box attacks](https://www.sciencedirect.com/science/article/abs/pii/S1568494621004294), No Code: Uses word embeddings with a substitute sequential (RNN) model. A joint model where the substitute (for target) and generative model (for generating attacks) shares parameters for efficient training. Tried both 'same' and 'diff' train datasets, coming from the same dataset.
