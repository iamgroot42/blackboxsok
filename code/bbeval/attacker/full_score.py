...
class Square_Attack(Attacker):
    ...
    def attack(self,x,y,eps,additional_params):
        p_init, n_iters, metrics_path = additional_params #TODO: merge the metrics_path into the logging function
        if self.norm_type == 'L_infty':
            x_adv, num_queries = self.square_attack_l2(x, y, eps, n_iters, p_init, metrics_path)
        elif self.norm_type == 'L_2':
            x_adv, num_queries = self.square_attack_linf(x, y, eps, n_iters, p_init, metrics_path)
        else:
    ...
    def square_attack_l2(self, x, y, eps, n_iters, p_init, metrics_path):
        ...
        s = h // 5
        log.print('Initial square side={} for bumps'.format(s)) #TODO: merge this part into the logging system
        sp_init = (h - s * 5) // 2
        center_h = sp_init + 0
        ...
        probs = self.model.predict(x_best) # TODO: this should return the prediction confidence score, sync with Anshuman, originally, it was logits
        loss_min = self.model.loss(y, probs, self.targeted, loss_type=self.loss_type) # TODO: sync with Anshuman, the model should contain a loss computation function that support different loss functions
        margin_min = self.model.loss(y, probs, self.targeted, loss_type='margin_loss') # TODO: similar to above
        ...
        for i_iter in range(n_iters):
            ...
            loss = self.model.loss(y_curr, probs, self.targeted, loss_type=self.loss_type)
            margin = self.model.loss(y_curr, probs, self.targeted, loss_type='margin_loss')
    ...
    def square_attack_linf(self, x, y, eps, n_iters, p_init, metrics_path):
        ...
        loss_min = self.model.loss(y, probs, self.targeted, loss_type=self.loss_type)
        margin_min = self.model.loss(y, probs, self.targeted, loss_type='margin_loss')
        n_queries = np.ones(x.shape[0])  # ones because we have already used 1 query
