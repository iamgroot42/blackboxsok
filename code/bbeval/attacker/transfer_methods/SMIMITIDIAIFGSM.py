...
from bbeval.attacker.transfer_methods._manipulate_input import input_diversity, transformation_function, clip_by_tensor
...
class SMIMITIDIAIFGSM(Attacker):
    ...
    def attack(self, x_orig, x_adv=None, y_label=None, x_target=None, y_target=None, experiment_file_name=None):
            ...
            for t in range(num_transformations):
                ...
                for s in range(size):
                    ...
                    for j in ch.arange(m):
                        ...
                        for model_name in self.aux_models:
                            model = self.aux_models[model_name]
                            output += model.forward(input_diversity(transformation_function(x_nes), image_resizes[0])) / n_model_ensemble

                        output_clone = output.clone()
                        loss = self.criterion(output_clone, y_target)
                        # print(loss)
                        loss.backward()
                        grad_temp += x_nes.grad.data/m/size
        ...
        #     num_transfered = ch.count_nonzero(target_model_prediction != y_target)
        # transferability = float(num_transfered / batch_size) * 100
        # print("The transferbility of SMIMITIDIAIFGSM is %s %%" % str(transferability))
